...
model2gen <- c("/Trial_poisson_beta_error", "/Trial_NB_beta_error_phiuniform")
title_gen <- c("Poisson(beta,error)", "NB(beta,error,phi1)")
model2inf <- c("/Trial_poisson_beta_error", "/Trial_NB_beta_error_phiuniform")
title_inf <- c("Poisson(beta,error)", "NB(beta,error,phi_uniform)")
...
# Model directory
dir_model <- paste0(dir_local, "/Models/Trial")

# Loading plotting inferences functions
source(paste0(dir_local, "/BayesWorkflow/plotting_inferences.R"))

# Simulating and inferring for each of the models
for (k in c(1:length(model2gen))) {
  ...
  simu_data <- list("G" = G, "S" = S, "design" = design)
  # Sampling with Stan
  fit_ensemble <- stan(file=paste0(dir_model, model2gen[k], "_ensemble.stan"),
  ...
  for (j in c(1:length(model2inf))) {
    input_data <- list("G" = G, "S" = S, "expression" = expression_real, "design" = design)
    options(mc.cores = 4)
    mc.cores = parallel::detectCores()
    nchains <- 4
    iter_per_chain <- 2000
    fit <- stan(file=paste0(dir_model, model2inf[j], "_post_pred.stan"),
    ...
    for (parameters in par_names) {
      title <- paste0("Simulation_", title_gen[k], "-Inference_", title_inf[j],
