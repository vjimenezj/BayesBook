...
model {
  ...
  for (i in 1:G) {
    expression[i, 1] ~ poisson_log(alpha[i] + beta[i] * design2[1] + error[i, 1] + eff_log_l_g[i]);
    for (j in 2:S) {
      expression[i, j] ~ poisson_log(alpha[i] + beta[i] * design2[j] + error[i, j] + eff_log_l_g[i] + log_norm_factors[j-1]);
    }
  }
}
