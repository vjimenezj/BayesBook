...
model2gen <- c("/BayesBook_poisson")
title_gen <- c("Poisson(beta,error)")
model2inf <- c("/BayesBook_poisson")
title_inf <- c("Poisson(beta,error)")
...
# Model directory
dir_model <- paste0(dir_local, "/Models/BayesBook")
...
for (k in c(1:length(model2gen))) {
  ...
  S <- 8
  C <- 2
  log_l_g <- rnorm(G, 1000, 10)
  # Data list for sample generation
  simu_data <- list("G" = G, "S" = S, "design" = design, "log_l_g" = log_l_g)
  # Sampling with Stan
  fit_ensemble <- stan(file=paste0(dir_model, model2gen[k], "_simulation.stan"),
  ...
  for (j in c(1:length(model2inf))) {
    ...
    nchains <- 4
    iter_per_chain <- 2000
    fit <- stan(file=paste0(dir_model, model2inf[j], "_inference.stan"),
